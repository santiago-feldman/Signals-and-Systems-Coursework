...
path = "Mediciones/BR1.csv"

def hideal(n, wc1, wc2, ws):
    def _hideal(n, wc1, wc2, ws):
        T = 2*np.pi/ws
        if n == 0:
            return (2/ws)*(wc1-wc2)+1
        else:
            return (np.sin(n*wc1*T)-np.sin(n*wc2*T))/(n*np.pi)
    result = []
    for i in n:
        result.append(_hideal(i, wc1, wc2, ws))
...
BWt1 = np.pi*2*(Fam-Fpm)
BWt2 = np.pi*2*(Fpp-Fap)
BWt = np.min([BWt1, BWt2])
wc1 = 2*np.pi*Fpm + BWt1/2
wc2 = 2*np.pi*Fpp - BWt2/2
...
hreal = hideal(n, wc1, wc2, np.pi*2*Fs)*window
