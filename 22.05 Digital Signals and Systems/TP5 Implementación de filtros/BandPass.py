...
def hideal(n, wc1, wc2, ws):
    def _hideal(n, wc1, wc2, ws):
        ...
        if n == 0:
            ...
            return (np.sin(n*wc2*T)-np.sin(n*wc1*T))/n/np.pi
    result = []
    for i in n:
...
# Normalizo la plantilla
BWt = np.pi*2*np.min([Fpm-Fam, Fap-Fpp])
wc1 = np.pi*2*Fpm - BWt/2
wc2 = np.pi*2*Fpp + BWt/2
